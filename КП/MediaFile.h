...
public:
    ...
    int getSize()   const;
    virtual int getDuration() const { return 0; }          // для Audio/Video
    virtual std::string getAlbum() const { return std::string(); }       // для Audio
    virtual std::string getResolution() const { return std::string(); }  // для Video/Image
    friend std::ostream& operator<<(std::ostream& os, const MediaFile& f);
    friend bool operator<(const MediaFile& a, const MediaFile& b);
};
